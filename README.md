...
| 局域网访问 | 默认**仅本机可访问**。想让手机 / 平板也能连：托盘右键 →「端口与局域网设置…」，勾选「允许局域网访问」，界面会直接列出该用哪个地址 |
...
| `HOST` | `127.0.0.1` | 监听地址。`Start-LAN.bat` 会设为 `0.0.0.0`；桌面版用「允许局域网访问」开关（写进 `desktop-config.json` 的 `allowLan`）。环境变量优先级最高 |
